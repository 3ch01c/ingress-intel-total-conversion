...
window.plugin.portalslist.stats = function(sortBy) {
  //console.log('** stats');
  var filterDiv = '<div><select id="displayType"><option value="1">Hide</option><option value="0">Show</option></select> portals with '
...
//hide portals that the agent(s) has resos on
window.plugin.portalslist.filterPlayer = function(player) {
	...
	$('#portalslist .res, #portalslist .enl').each(function (ind, val) {
		var count = 0; // increment for every agent found
		for (var i = 0; i < players.length; i++) {
			player = players[i];
			var lvl = minLvl;
			var playerFound = false;
			// check for resos owned by agent
			while (!playerFound && lvl <= maxLvl) {
				if ($(this).find('.L'+lvl+'[title^="owner: <b>'+player+'</b>"]').length > 0) {
					count++;
					playerFound = true;
				} else {
					// check the next reso level
					lvl++;
				}
			}
		}
		// if all agents have reso(s) on this portal
